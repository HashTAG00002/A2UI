...
由 runtime probe 在 bf60e0c 执行验证过，真实且 Kernel-owned：
...
| GoalPatch two-phase（invalidate future + block until recompose） | **加**（v4 缺，是真 bug） |
| `_TRANSITIONS` READY→FAILED（VERIFY only） | **加** |
| workflow rewind after rollback（frontier/loop/timeline） | **加**（v4 缺） |
| CHECKPOINT self-in-snapshot + stable-boundary | **加/修** |
| set_plan/init one-shot guard | **加**（用 explicit initialized flag，不用"variables 非空"） |
