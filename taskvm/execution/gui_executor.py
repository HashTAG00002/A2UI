...
# GG.3: SubgoalGenerator replaces the hardcoded _build_instruction template
# (which leaked entity_id via "the {entity_kind} with id '{entity_id}'"). The
# instruction is now generated from the visible locator (control-plane
# translated from canonical state) — zero internal id, zero operator jargon.
...
def gui_write(*, app: str, sid: str, entity_id: str, operator: str, value: Any,
              ...
              backend_name: str = DEFAULT_BACKEND,
              canonical_entities: dict[str, dict[str, Any]] | None = None,
              instruction: str | None = None,
              mock_subgoal: bool = True) -> dict:
    ...
    GG.3: the instruction fed to the grounding model is generated by the
    SubgoalGenerator from the entity's VISIBLE LOCATOR (translated control-plane
    from ``canonical_entities``) — zero internal id, zero operator jargon. The
    old ``_build_instruction`` (which leaked "the {entity_kind} with id
    '{entity_id}'") is deleted. Two ways to supply the instruction:
      - ``instruction`` (caller-supplied, GG.3 "封死旧路径"): if given, used
        verbatim (the governance layer's SubgoalInstruction.natural_language).
      - ``canonical_entities``: if ``instruction`` is None, the SubgoalGenerator
        builds the NL from the op + the entity's visible locator in this dict.
      - ``mock_subgoal``: True (default) → deterministic mock NL (no LLM call,
        for offline/reproducible runs); False → LLM path.
    ...
    if instruction is None:
        op = PatchOp(app=app, entity_id=entity_id, field=field,
                     operator=operator, value=value)
        instruction, _meta = instruction_for_op(
            op, canonical_entities or {}, undo=undo, attempt=attempt,
            mock=mock_subgoal)
