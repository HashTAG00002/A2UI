...
def test_gui_driver_write_plane():
    ...
    still exists — the legacy phase scripts used it (now deleted; the
    workspace-UI stack remains its caller until Agent G's wave) — but this
    test no
...
def test_compiler_vision_signature():
    ...
    encoder); default None keeps the text path. (The legacy W1 entry that
    consumed the flag is deleted with the phase scripts; the signature is
...
# ── FF.2: UISimDriver (fakes; GUI-only writes) ────────
