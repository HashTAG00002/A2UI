...
test_kernel_invariants.py / test_scenario.py; NOTHING here replaces
those — this file only raises the bar.
"""
import pytest
...
def test_old_action_cannot_mutate_committed_state():
    ...
    # a GoalPatch lands mid-flight (carrying a1 verbatim into the new plan)
    carried = k.workflow().graph.node("a1")
    k.apply_goal_patch(
        GoalPatch(patch_id="gp", new_intent=TaskIntent(goal="goal-v2")),
...
# ══ 7. rollback restores the checkpoint's TASK STRUCTURE ══════════════════
def test_rollback_restores_checkpoint_task_structure():
    k = _kernel((
        _var("x", 1, 1, label="X 变量"),
        _var("y", 2, 2, label="Y 标签", value_type="date"),
    ), goal="g")
    k.commit_checkpoint("C0", "结构边界")
    k.apply_observation([_obs("x", 10)])
    ...
    assert targets == {"x": 1, "y": 2}       # y restored from absence
    assert k.record_compensation_result(
        plan.plan_id, applied=True,
        observed_values={"x": 1, "y": 2}) is True
    state = k.task_state()
    assert state.variable("x").observed == 1
    y = state.variable("y")
    assert y is not None and y.label == "Y 标签"   # structure, not just value
    assert y.value_type == "date" and y.observed == 2 and y.desired == 2
    assert state.variable("z") is None             # abandoned future dropped
    ev = [e for e in k.events()
          if e.kind is EventKind.COMPENSATION_APPLIED][-1]
    assert ev.payload["restored_structure_keys"] == ["y"]
    assert ev.payload["dropped_semantic_keys"] == ["z"]
...
def test_local_patch_rejects_duplicate_variable_updates():
    ...
    with pytest.raises(ValidationError, match="duplicate"):
        LocalPatch(patch_id="lp_dup2",
                   node_overrides=(
                       NodeContractOverride("a1", _contract("c1", "x", 1)),
                       NodeContractOverride("a1", _contract("c2", "x", 2))))
