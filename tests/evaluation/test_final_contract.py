...
from taskvm.benchmark.registry import Condition
from taskvm.benchmark.schema import Family, Injection, InjectionKind, Split, TaskSpec
...
#: The legacy source files that still import taskvm.benchmark.* — every
#: one is a registered transitional-debt item owned by Agent G Wave-3 /
...
def test_runtime_import_graph_clean():
    ...
    code = (
        ...
        "       if m.startswith(('taskvm.benchmark', 'taskvm.evaluation'))]\n"
        "print(','.join(bad))\n"
    ...
    assert r.returncode == 0, (
        f"modern runtime plane imports benchmark/evaluation: "
        f"{r.stdout.strip()}")


def test_legacy_benchmark_import_register_shrink_only():
    """The set of source files importing taskvm.benchmark.* outside the
    evaluation plane equals the registered debt — no new offenders."""
    offenders = set()
    for dirpath, dirnames, filenames in os.walk(os.path.join(REPO, "taskvm")):
        ...
        for name in filenames:
            ...
            if rel.startswith("taskvm/benchmark") or rel.startswith(
                    "taskvm/evaluation") or rel.startswith("taskvm/baselines"):
                continue                     # the evaluation plane itself
            with open(path, encoding="utf-8", errors="replace") as f:
                src = f.read()
            if re.search(r"\b(?:from|import)\s+taskvm\.benchmark\b", src):
                offenders.add(rel)
...
def test_oracle_secrets_never_reach_the_system_under_test(monkeypatch):
    ...
    from taskvm.evaluation import harness as H
...
def test_same_seed_same_trial():
    from taskvm.benchmark.tasks import get_task
...
def test_world_and_oracle_cannot_reach_production():
    ...
    for mod in ("world", "oracle"):
        path = os.path.join(REPO, "taskvm", "evaluation", f"{mod}.py")
...
def test_injections_route_only_through_public_seams():
    ...
    import ast
    path = os.path.join(REPO, "taskvm", "evaluation", "harness.py")
    with open(path, encoding="utf-8") as f:
...
def test_environment_writes_cannot_advance_injection_milestones():
    ...
    from taskvm.evaluation.world import ENVIRONMENT, BenchmarkWorld
    spec = TaskSpec(
